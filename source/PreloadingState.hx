...
class PreloadingState extends MusicBeatState
{
	...
	// Browsers will load create(), so I can make a custom Create.
	...
	override function create()
	{
		...
		funkay.updateHitbox();
		funkay.antialiasing = utilities.Options.getData("antialiasing");
		add(funkay);
		...
		loadBar.antialiasing = utilities.Options.getData("antialiasing");
		add(loadBar);
		
		initSongsManifest().onComplete
		(
            //alr lets get loading.
			function (lib)
			{
				callbacks = new MultiCallback(onLoad);
				var introComplete = callbacks.add("introComplete");
                //first. lets load all the song files for freeplay.
   ...
   /* function loadFile(file:String) //used to load singular files. quick for specific files
	...
	{
		return Paths.inst(song);
	}
	
	static function getVocalPath(song:String)
	{
		return Paths.voices(song);
	}
